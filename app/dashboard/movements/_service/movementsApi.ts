...
export const movementsApi = createApi({
  ...
  endpoints: (builder) => ({
    getMovements: builder.query<
      PaginatedResult<InventoryMovementResponse>,
      GetMovementsArgs
    >({
      query: (arg) => {
    ...
    getMovementFormContext: builder.query<MovementFormContext, void>({
      ...
      transformResponse: (
        raw: MovementFormContext | { result?: MovementFormContext },
      ) =>
        (raw && typeof raw === "object" && "result" in raw
          ? raw.result
          : raw) as MovementFormContext,
    }),
    ...
    >({
      ...
      transformResponse: (
        raw: InventoryMovementResponse | { data: InventoryMovementResponse },
    ...
    getMovementStats: builder.query<
      ...
      { from?: string; to?: string; today?: boolean } | undefined
    >({
      query: (arg) => {
    ...
    getFlowWithCumulative: builder.query<
      { label: string; value: number; lineValue?: number }[],
      { days?: number } | undefined
    >({
      query: (arg) =>
        `/inventory-movement/flow-with-cumulative?days=${(arg as { days?: number })?.days ?? 7}`,
      transformResponse: (raw: unknown) =>
        parseChartResult<{ label: string; value: number; lineValue?: number }>(
          raw,
        ),
    }),
    getDistributionByType: builder.query<
